...
func main() {
	log.SetPrefix("sms-wkr ")
	log.SetFlags(log.Lshortfile | log.LstdFlags)
	log.Println("SMS worker running")
	...
	// Durable queue of name sms
	queue, err := ch.QueueDeclare("sms", true, false, false, false, nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	go func() {
		for d := range messages {
			...
			log.Println("SMS-ing '" + em.Recepient + "' with content '" + em.Message + "'")
	...
	log.Println("SMS worker listening for messages")
	<-forever
}
